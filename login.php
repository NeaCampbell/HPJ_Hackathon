...
if (isset($_POST['login'])) {
    ...
    $check = "SELECT user_id, user_password, user_level FROM `user` WHERE user_name = ?";
    $check = $connPDO->prepare($check);
    $check->execute([$username]);
    $fetch = $check->fetch();

    if ($check->rowCount() == 0) {
        $msg = 'Username salah';
    } else if (password_verify($password, $fetch['user_password'])) {
        $msg = 'Password salah';
    } else {
        $_SESSION['login'] = $fetch['user_id'];
        $_SESSION['user_level'] = $fetch['user_level'];
        
        if ($fetch['user_level'] == 'admin') {
        ...
        <div class="content">
            <form action="" method = "POST">
                <div class = "container">
                    ...
                    <input type="text" placeholder="Enter Username" name="logname" required>

                    <label for="psw"><b>Password</b></label>
                    <input type="password" placeholder="Enter Password" name="logpass" required>

                    <button name="login" class ="btn">Login</button>
                    <span class="psw1">Belum punya akun? <a href="Sign.php">Sign Up</a></span>
                </div>
            </form>
        </div>
    </div>
